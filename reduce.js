...
function extractValue(arr, key) {
    return arr.reduce(function(accum, nextName) {
        accum.push(nextName[key]); // only adding the value to new array
        return accum;
    },[]);
}
...
function partition(arr, callback) {
    return arr.reduce(function(accum,next){
        if (callback(next)) {
            accum[0].push(next); // if it's true, add it to the first array
        }
        else {
            accum[1].push(next); // if it's false, add it to the second array
        }
        return accum;
    }, [[],[]])
}
